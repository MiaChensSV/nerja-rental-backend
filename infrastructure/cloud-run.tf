...
resource google_cloud_run_service "backend" {
  name = "nerja-rental-backend-${var.environment_short}"
  location = "${var.region}"
  ...
  template {
    spec {
      ...
      containers {
        image = "europe-west10-docker.pkg.dev/${var.project_id}/${var.project_id}-frontend-${var.environment_short}/frontend-image:latest"
        ports {
          container_port = 3001
        }
        resources {
